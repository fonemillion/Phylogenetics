...
def greedyPick_4old(tree_set, isTree=True):
    if isTree:
            ...
            if len(case.clusters) > 0:
                ...
                if len(case.node_dict[pick]) == 1:
                    ...
                    if case.node_dict[pick] in clusters_solved:
                        ...
                        _, sol_partial = greedyPick_4(case.get_cluster(pick), isTree=False)
...
def greedyPick_4(tree_set, isTree=True):
    if isTree:
        main_case = Case_T(tree_set, input_is_tree=True)
    else:
        main_case = tree_set
